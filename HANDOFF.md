...
> [!IMPORTANT]
> **この文書は MVP 時点の記録であり、下の「現状」と「TODO 候補」は既に古い。**
> 引き継ぎプロンプト内の「現状 (= MVP, 重複検知なし, 観測性最小)」は当時の姿で、
> 現在はそうではない。TODO 候補 7 件の消化状況は次のとおり。
>
> | # | TODO 候補 | 状況 | 実体 |
> |---|---|---|---|
> | 1 | URL 重複検知 | 実装済み | ADR 0003 / 0010、`src/url-index.ts` |
> | 2 | 要約モデルの切り替え | 実装済み | `src/llm.ts`（`SUMMARY_PROVIDER=anthropic`） |
> | 3 | 本文取得の堅牢化 | 実装済み | ADR 0007、`src/fetch-article.ts` |
> | 4 | タグ自動付与 | 実装済み | ADR 0008、`src/tags.ts` |
> | 5 | 失敗通知 | 実装済み | ADR 0006、`src/notify.ts` |
> | 6 | テスト | 実装済み | ADR 0001（vitest）、`src/index.test.ts` |
> | 7 | 観測性 | **未着手** | — |
>
> **現在の設計判断の正典は `docs/adr/`。** この文書を未実装リストとして読まないこと。
> 一方、要約モデル比較ハーネスの検証結果 (#16 / #17) と Claude Agent SDK を
> 採用しなかった経緯は、ここにしか残っていない情報なので保存している。
