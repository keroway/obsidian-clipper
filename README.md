...
- 📥 **1 タップで保存** — iOS ショートカット / Chrome ブックマークレット / `curl` から `POST /clip` を叩くだけ。
...
- 🪶 **本文抽出は Jina Reader** — SPA / 動的サイトでもそれなりに動く。`JINA_API_KEY` を入れれば rate limit が緩む。
...
| `tags`       | `string[]` | -    | 追加タグ。`clipped` は常に自動付与                                |
...
> **失敗ポリシー**: Jina Reader / 要約のどちらが失敗しても 200 を返します。本文が取れなかった場合は本文セクションにエラー理由が残るだけで、URL とユーザメモは保存されます。
...
- **本文抽出の限界**: Jina Reader は媒体記事には強いものの、SPA で JS 後挿入される本文や paywall コンテンツは取れません。代替経路として [Cloudflare Browser Rendering API](https://developers.cloudflare.com/browser-rendering/) が候補です。
...
- Jina Reader 障害時の Readability / Browser Rendering フォールバック
- LLM によるタグ自動付与
